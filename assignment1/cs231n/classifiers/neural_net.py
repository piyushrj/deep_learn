...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    zeros_mat = np.zeros(shape=dscores.shape)
    zeros_mat[np.arange(len(zeros_mat)), y] = 1
    dscores -= zeros_mat
    # Derivative w.r.t second bias term
    db2 = 1*np.sum(dscores, axis=0)/N
    grads['b2'] = db2
    # Derivative w.r.t W2
    dW2 = first_output.T.dot(dscores) / N
    dW2 += 2*reg*W2
    grads['W2'] = dW2
    # Derivative w.r.t first_output(forward from relu) dfo
    dfo = dscores.dot(W2.T) * (fo > 0)
    #dfo[fo < 0] = 0
    # Derivative w.r.t first bias b1
    db1 = 1*np.sum(dfo, axis=0)/N
    grads['b1'] = db1
    # Derivative w.r.t W1
    dW1 = X.T.dot(dfo) / N
    dW1 += 2*reg*W1
